...
class OrganizationsAPI(BaseAPI[Organization]):
        ...
        **kwargs
        ...
        if isinstance(status, OrganizationStatus):
            ...
            try:
                ...
                raise ITGlueValidationError(f"Invalid organization status: {status_value}")
    ...
    async def bulk_update_status(
        ...
        """
        logger.info(f"Bulk updating {len(organization_ids)} organizations to status: {status}")
        ...
        logger.info(f"Successfully updated {len(updated_organizations)} of {len(organization_ids)} organizations")
