...
class BaseAPI(Generic[T]):
        ...
        **kwargs
        ...
        try:
            ...
            if e.status_code == 404:
                raise ITGlueNotFoundError(f"{self.resource_type.value.title()} {resource_id} not found")
            raise
        ...
        **kwargs
        ...
        if isinstance(data, self.model_class):
            ...
            try:
                ...
                raise ITGlueValidationError(f"Invalid {self.resource_type.value} data: {e}")
    ...
    async def delete(self, resource_id: str, **kwargs) -> None:
        ...
        try:
            ...
            logger.info(f"Successfully deleted {self.resource_type.value} {resource_id}")
        except ITGlueAPIError as e:
            if e.status_code == 404:
                raise ITGlueNotFoundError(f"{self.resource_type.value.title()} {resource_id} not found")
            raise
