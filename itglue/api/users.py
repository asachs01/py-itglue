...
class UsersAPI(BaseAPI[User]):
    ...
    async def get_active_users(self, **kwargs) -> List[User]:
        ...
        try:
            ...
            return [user for user in result if user.is_active()] if isinstance(result, list) else []
    ...
    async def get_invited_users(self, **kwargs) -> List[User]:
        ...
        try:
            ...
            return [user for user in result if user.is_invited()] if isinstance(result, list) else []
    ...
    async def get_my_glue_users(self, **kwargs) -> List[User]:
        ...
        try:
            ...
            return [user for user in result if user.has_my_glue_access()] if isinstance(result, list) else []
    ...
    async def get_recently_active_users(self, days: int = 30, **kwargs) -> List[User]:
        ...
        try:
            ...
            raise ITGlueAPIError(f"Failed to get recently active users: {str(e)}") from e
    ...
    async def update_user_role(self, user_id: str, role: Union[str, UserRole], **kwargs) -> User:
        """
    ...
    async def update_user_profile(self, user_id: str, profile_data: Dict[str, Any], **kwargs) -> User:
        """
    ...
    async def get_user_statistics(self, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(all_users, UserCollection):
                ...
                collection = UserCollection(all_users if isinstance(all_users, list) else [])
            ...
            return {
                ...
                "average_reputation": round(
                    sum(user.reputation or 0 for user in collection) / len(collection)
                    if len(collection) > 0 else 0, 2
                )
            }
