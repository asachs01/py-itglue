...
class FlexibleAssetField(ITGlueResource):
    ...
    def __str__(self) -> str:
        """String representation."""
        return f"FlexibleAssetField(id={self.id}, name='{self.name}', kind='{self.kind}')"
...
class FlexibleAssetCollection(ITGlueResourceCollection[FlexibleAsset]):
    ...
    def get_by_type(self, type_name: str) -> List[FlexibleAsset]:
        ...
        return [
            asset for asset in self.data 
            if asset.flexible_asset_type_name and asset.flexible_asset_type_name.lower() == type_name.lower()
        ]
    ...
    def get_by_trait(self, trait_name: str, trait_value: Any = None) -> List[FlexibleAsset]:
...
class FlexibleAssetFieldCollection(ITGlueResourceCollection[FlexibleAssetField]):
    ...
    def get_by_kind(self, kind: str) -> List[FlexibleAssetField]:
        ...
        return [field for field in self.data if field.kind and field.kind.lower() == kind.lower()] 