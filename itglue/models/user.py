...
class User(ITGlueResource):
    ...
    def is_invited(self) -> bool:
        ...
        return (self.invitation_sent_at is not None and 
...
class UserCollection(ITGlueResourceCollection[User]):
    ...
    def find_by_name(self, name: str) -> List[User]:
        ...
        for user in self.data:
            if (user.full_name.lower().find(name_lower) != -1 or
                (user.first_name and user.first_name.lower().find(name_lower) != -1) or
                (user.last_name and user.last_name.lower().find(name_lower) != -1)):
                matches.append(user)
    ...
    def get_my_glue_statistics(self) -> Dict[str, int]:
        ...
        return {
            ...
            "my_glue_adoption_percentage": round((with_access / total * 100) if total > 0 else 0, 2)
        }
