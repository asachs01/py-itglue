...
class TestUserModel:
    ...
    def test_user_activity_days_calculation(self):
        ...
        user.set_attribute('last-sign-in-at', datetime.now(timezone.utc) - timedelta(days=1))
        days = user.calculate_activity_days()
        assert days == 1
        
        # Older activity (30 days ago)
        user.set_attribute('last-sign-in-at', datetime.now(timezone.utc) - timedelta(days=30))
        days = user.calculate_activity_days()
        assert days == 30
...
class TestUserCollection:
    ...
    def test_collection_add_remove(self):
        ...
        collection = UserCollection()
        user = User.from_api_dict({"id": "999", "type": "users", "attributes": {"first-name": "Test"}})
    ...
    def test_get_recently_active_users(self):
        ...
        recent_user_data = {
            ...
            "attributes": {
                "first-name": "Recent",
                "last-sign-in-at": (datetime.now(timezone.utc) - timedelta(days=5)).isoformat()
            }
        }
        ...
        old_user_data = {
            ...
            "attributes": {
                "first-name": "Old",
                "last-sign-in-at": (datetime.now(timezone.utc) - timedelta(days=60)).isoformat()
            }
        }
