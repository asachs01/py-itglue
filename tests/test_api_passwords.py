...
from itglue.models.password import Password, PasswordCollection, PasswordType, PasswordCategory, PasswordVisibility
...
def sample_password_data():
    ...
    return {
        ...
        "relationships": {
            "organization": {
                "data": {"type": "organizations", "id": "456"}
            }
        }
    }
...
class TestPasswordsAPISearch:
    ...
    @pytest.mark.asyncio
    async def test_search_passwords(self, passwords_api, sample_password_collection_data):
        """Test searching passwords by query."""
        passwords_api.list = AsyncMock(return_value=[Password.from_api_dict(sample_password_collection_data["data"][0])])
        
        results = await passwords_api.search_passwords("Gmail", organization_id="456")
        
        passwords_api.list.assert_called_once_with(params={
            "filter[name]": "Gmail",
    ...
    @pytest.mark.asyncio
    async def test_get_by_name_exact_match(self, passwords_api, sample_password_collection_data):
        """Test getting password by exact name match."""
        password_data = sample_password_collection_data["data"][0]
        passwords_api.list = AsyncMock(return_value=[Password.from_api_dict(password_data)])
    ...
    @pytest.mark.asyncio
    async def test_get_by_name_case_insensitive(self, passwords_api, sample_password_collection_data):
    ...
    @pytest.mark.asyncio
    async def test_search_by_username(self, passwords_api, sample_password_collection_data):
        """Test searching passwords by username."""
        password_data = sample_password_collection_data["data"][0]
        passwords_api.list = AsyncMock(return_value=[Password.from_api_dict(password_data)])
        
        results = await passwords_api.search_by_username("user1@example.com", organization_id="456")
...
class TestPasswordsAPIOrganizationFiltering:
    ...
    @pytest.mark.asyncio
    async def test_get_organization_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting passwords for an organization."""
        passwords_api.list = AsyncMock(return_value=[
            Password.from_api_dict(data) for data in sample_password_collection_data["data"]
        ])
        
        results = await passwords_api.get_organization_passwords("456", include_archived=False)
    ...
    @pytest.mark.asyncio
    async def test_get_organization_passwords_include_archived(self, passwords_api, sample_password_collection_data):
        """Test getting organization passwords including archived."""
        passwords_api.list = AsyncMock(return_value=[
            Password.from_api_dict(data) for data in sample_password_collection_data["data"]
        ])
        
        results = await passwords_api.get_organization_passwords("456", include_archived=True)
...
class TestPasswordsAPISecurityFiltering:
    ...
    @pytest.mark.asyncio
    async def test_filter_by_category_enum(self, passwords_api, sample_password_collection_data):
        """Test filtering by password category using enum."""
        password_data = sample_password_collection_data["data"][1]  # critical password
        passwords_api.list = AsyncMock(return_value=[Password.from_api_dict(password_data)])
        
        results = await passwords_api.filter_by_category(PasswordCategory.CRITICAL, organization_id="456")
    ...
    @pytest.mark.asyncio
    async def test_filter_by_category_string(self, passwords_api, sample_password_collection_data):
        ...
        passwords_api.list.assert_called_once_with(params={
            "filter[password-category-name]": "high"
        })
        assert len(results) == 1
    
    @pytest.mark.asyncio
    async def test_filter_by_visibility_enum(self, passwords_api, sample_password_collection_data):
        """Test filtering by visibility using enum."""
        password_data = sample_password_collection_data["data"][0]  # private password
        passwords_api.list = AsyncMock(return_value=[Password.from_api_dict(password_data)])
        
        results = await passwords_api.filter_by_visibility(PasswordVisibility.PRIVATE, organization_id="456")
    ...
    @pytest.mark.asyncio
    async def test_filter_by_type_enum(self, passwords_api, sample_password_collection_data):
...
class TestPasswordsAPISecurityMethods:
    ...
    @pytest.mark.asyncio
    async def test_get_critical_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting critical passwords."""
        password_data = sample_password_collection_data["data"][1]  # critical password
        passwords_api.filter_by_category = AsyncMock(return_value=[Password.from_api_dict(password_data)])
    ...
    @pytest.mark.asyncio
    async def test_get_high_security_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting high security passwords."""
        high_password = Password.from_api_dict(sample_password_collection_data["data"][0])
        critical_password = Password.from_api_dict(sample_password_collection_data["data"][1])
        
        passwords_api.filter_by_category = AsyncMock(side_effect=[
    ...
    @pytest.mark.asyncio
    async def test_get_high_security_passwords_deduplication(self, passwords_api, sample_password_collection_data):
        ...
        same_password = Password.from_api_dict(sample_password_collection_data["data"][0])
    ...
    @pytest.mark.asyncio
    async def test_get_shared_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting shared passwords."""
        shared_password = Password.from_api_dict(sample_password_collection_data["data"][1])  # organization visibility
        
        passwords_api.filter_by_visibility = AsyncMock(side_effect=[
            [],               # shared visibility
    ...
    @pytest.mark.asyncio
    async def test_get_private_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting private passwords."""
        private_password = Password.from_api_dict(sample_password_collection_data["data"][0])
    ...
    @pytest.mark.asyncio
    async def test_get_embedded_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting embedded passwords."""
        embedded_password = Password.from_api_dict(sample_password_collection_data["data"][0])
    ...
    @pytest.mark.asyncio
    async def test_get_linked_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting linked passwords."""
        linked_password = Password.from_api_dict(sample_password_collection_data["data"][1])
...
class TestPasswordsAPISpecialStates:
    ...
    @pytest.mark.asyncio
    async def test_get_favorite_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting favorite passwords."""
        favorite_password = Password.from_api_dict(sample_password_collection_data["data"][0])
    ...
    @pytest.mark.asyncio
    async def test_get_archived_passwords(self, passwords_api, sample_password_collection_data):
    ...
    @pytest.mark.asyncio
    async def test_get_active_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting active passwords."""
        active_password = Password.from_api_dict(sample_password_collection_data["data"][0])
...
class TestPasswordsAPITimeFiltering:
    ...
    @pytest.mark.asyncio
    async def test_get_recently_updated_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting recently updated passwords."""
        recent_password = Password.from_api_dict(sample_password_collection_data["data"][0])
        passwords_api.list = AsyncMock(return_value=[recent_password])
        
        results = await passwords_api.get_recently_updated_passwords(30, organization_id="456")
    ...
    @pytest.mark.asyncio
    async def test_get_stale_passwords(self, passwords_api, sample_password_collection_data):
        """Test getting stale passwords."""
        stale_password = Password.from_api_dict(sample_password_collection_data["data"][0])
...
class TestPasswordsAPIManagement:
    ...
    async def test_create_password(self, passwords_api, sample_password_data):
        ...
        passwords_api.create = AsyncMock(return_value=Password.from_api_dict(sample_password_data))
    ...
    @pytest.mark.asyncio
    async def test_create_password_with_string_enums(self, passwords_api, sample_password_data):
    ...
    async def test_update_password_value(self, passwords_api, sample_password_data):
        ...
        passwords_api.update = AsyncMock(return_value=Password.from_api_dict(sample_password_data))
    ...
    @pytest.mark.asyncio
    async def test_update_password_visibility(self, passwords_api, sample_password_data):
        """Test updating password visibility."""
        passwords_api.update = AsyncMock(return_value=Password.from_api_dict(sample_password_data))
        
        result = await passwords_api.update_password_visibility("123", PasswordVisibility.EVERYONE)
...
class TestPasswordsAPIAnalytics:
    ...
    @pytest.mark.asyncio
    async def test_get_password_statistics(self, passwords_api, sample_password_collection_data):
        """Test getting comprehensive password statistics."""
        active_passwords = [Password.from_api_dict(data) for data in sample_password_collection_data["data"]]
        archived_passwords = []
        
        passwords_api.get_active_passwords = AsyncMock(return_value=active_passwords)
        passwords_api.get_archived_passwords = AsyncMock(return_value=archived_passwords)
    ...
    @pytest.mark.asyncio
    async def test_get_organization_password_report(self, passwords_api, sample_password_collection_data):
        """Test getting comprehensive organization password report."""
        active_passwords = [Password.from_api_dict(data) for data in sample_password_collection_data["data"]]
        
        # Mock all the required method calls
        passwords_api.get_password_statistics = AsyncMock(return_value={
            "total_passwords": 2,
            "active_passwords": 2,
            "critical_passwords": 1,
            "shared_passwords": 1,
            "private_passwords": 1,
            "security_distribution": {"low": 0, "medium": 0, "high": 1, "critical": 1}
        })
        passwords_api.get_stale_passwords = AsyncMock(return_value=[])
        passwords_api.get_recently_updated_passwords = AsyncMock(return_value=active_passwords)
        ...
        assert recommendations["critical_review_needed"] is True  # Has critical passwords
        assert recommendations["stale_passwords_need_update"] is False  # No stale passwords
        assert recommendations["consider_upgrading_low_security"] is False  # No low security passwords
        assert recommendations["review_shared_passwords"] is False  # More private than shared 