...
from itglue.models.organization import Organization, OrganizationStatus, OrganizationTypeEnum
...
class TestOrganizationsAPISpecializedMethods:
    ...
    async def test_get_by_name_exact_match(self, organizations_api, mock_http_client, sample_organization_data):
        ...
        with patch.object(organizations_api, '_process_response') as mock_process:
            ...
            result = await organizations_api.get_by_name('Test Organization', exact_match=True)
            
            assert result == mock_org
            expected_params = {'filter[name]': 'Test Organization', 'page[size]': '1'}
            mock_http_client.get.assert_called_once_with('/organizations', params=expected_params)
    
    async def test_update_status_with_enum(self, organizations_api, mock_http_client, sample_organization_data):
        ...
        with patch.object(organizations_api, '_process_response') as mock_process:
            ...
            result = await organizations_api.update_status('123', OrganizationStatus.INACTIVE)
    ...
    async def test_create_organization_with_enum(self, organizations_api, mock_http_client, sample_organization_data):
